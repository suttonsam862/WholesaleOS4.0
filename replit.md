...
## Recent Changes
- **Manufacturer Portal Phase 1 UI Skeleton (2025-12-12)**: Added Hub → Queue → Detail navigation pattern for manufacturer portal as additive pages (no backend changes). Features include: (1) Config-driven funnel stage system in `client/src/lib/manufacturerFunnelConfig.ts` with zone definitions (intake, specs, samples, production, shipping), stage colors/icons, and allowed transitions, (2) ManufacturerQueuePage at `/manufacturer-portal/queue` with URL-driven zone/status filtering, (3) ManufacturerJobDetail page at `/manufacturer-portal/job/:id` with next action buttons based on config transitions, (4) Enhanced manufacturer home hub with zone-based workflow tiles and queue widgets for urgent/overdue jobs, (5) Legacy manufacturer portal preserved at `/manufacturer-portal`. All new routes use existing `/api/manufacturer-portal/jobs` endpoints.
- **Finance Hub Workflow Redesign (2025-12-12)**: Implemented workflow-first hub pattern for Finance page with progressive disclosure. Features include: (1) /finance now shows FinanceHub with workflow tiles, metrics snapshot, and quick actions, (2) Subroutes /finance/overview, /finance/invoices, /finance/payments, /finance/commissions, /finance/matching, /finance/expenses for deep navigation, (3) URL param support for action=new and status filters, (4) Legacy access preserved at /finance/legacy, (5) Reusable HubLayout, WorkflowTile, MetricsSnapshot components in client/src/components/workflow/. Pattern designed to be applied to Manufacturing, Design Jobs, and Leads pages.
