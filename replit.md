...
- **Action Framework**: A comprehensive wizard-based action system integrated across 7 hubs (orders, sales-analytics, organizations, contacts, leads, events, quotes). Features ActionDeck component with 4 pinned action cards per hub, action index pages (/<resource>/actions), and action detail pages (/<resource>/actions/<actionId>) with Pick → Choose → Preview → Confirm → Done wizard flow. Includes AI integration via Gemini API (POST /api/ai/interactions) using GEMINI_API_KEY for three pilot AI actions: "Client brief" (SA1), "Explain my numbers" (SA4), and "Make quote from order" (O1). All AI actions follow draft-first workflow where AI generates content for preview before user confirmation saves changes.
